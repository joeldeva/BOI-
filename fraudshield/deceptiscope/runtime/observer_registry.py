...
class ObserverRegistry:
    """Registry managing trusted, local defensive Frida observer scripts with policy flag enforcement."""
    ...
    def is_observer_enabled(self, observer_name: str) -> bool:
        """Returns True if the observer is an approved module and enabled by current settings."""
        ...
        return flag_map.get(observer_name, True)

    def get_observers_for_experiments(
        self,
        experiments: Sequence[ExperimentType],
    ) -> list[str]:
        """Returns deduplicated list of approved, configured-enabled observer module names."""
        selected: list[str] = []
        for exp in experiments:
            for obs in EXPERIMENT_OBSERVER_MAP.get(exp, ()):
                if obs in OBSERVER_SCRIPT_FILES and self.is_observer_enabled(obs) and obs not in selected:
                    selected.append(obs)
        return selected

    def load_observer_script(self, observer_name: str) -> str:
        """Loads a single trusted local observer script. Rejects unapproved or disabled observers."""
        if observer_name not in OBSERVER_SCRIPT_FILES:
            raise ValueError(f"Unknown or unapproved observer: {observer_name}")
        if not self.is_observer_enabled(observer_name):
            raise ValueError(f"Observer {observer_name} is disabled by configuration.")

        file_name = OBSERVER_SCRIPT_FILES[observer_name]
        script_path = (self.scripts_dir / file_name).resolve()

        # Enforce strict directory containment to prevent path traversal
        if script_path.parent != self.scripts_dir:
            raise ValueError(f"Security error: path traversal detected for observer {observer_name}")
    ...
    def build_bundle(self, observer_names: Sequence[str]) -> str:
        """Combines multiple trusted, enabled observer scripts into a single sandboxed bundle."""
        snippets: list[str] = []
        for name in observer_names:
            if name in OBSERVER_SCRIPT_FILES and self.is_observer_enabled(name):
                try:
